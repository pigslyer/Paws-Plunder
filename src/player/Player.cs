...
public partial class Player : CharacterBody3D, IBulletHittable, IDeathPlaneEnterable
{
    ...
    private void ApplyMovement(float fDelta, bool hasControls)
    {
        ...
        if (hasControls)
        {
            inputVector = new Vector2(
                Input.GetActionStrength("plr_right") - Input.GetActionStrength("plr_left"),
                Input.GetActionStrength("plr_back") - Input.GetActionStrength("plr_forward")
                ).Normalized();
        ...
        // camera offset
        {
            ...
            Vector3 headRotation = new(_head.Rotation.X, _head.Rotation.Y,
                float.Clamp(_head.Rotation.Z, -0.05f, 0.05f));
    ...
    private void MouseRotateCamera(float delta)
    {
        ...
        rotation += new Vector3(-_mouseMotion.Y, -_mouseMotion.X, 0) * SensitivityMult * delta *
                    Globals.MouseSensitivity;
    ...
    private void ShootAttack()
    {
        ...
        else if (_currentGun == GunTypes.Quad)
        {
            ...
            Globals.CalculateShotgunDirections(shotDirection, float.DegreesToRadians(30), QuadBulletVelocity,
                bulletVelocities);
        ...
        if (_remainingAmmo == 0)
        {
            _gunAnimationPlayer.Connect("animation_finished", Callable.From<string>(OnGunAnimationFinished),
