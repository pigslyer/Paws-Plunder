...
public partial class Player : CharacterBody3D, IBulletHittable, IDeathPlaneEnterable
{
    ...
    private void MeleeAttack()
    {
        ...
        JustFired = true;
        Godot.Collections.Array<Godot.Node3D> bodies = _meleeDetectionArea.GetOverlappingBodies();
    ...
    private void ShootAttack()
    {
        ...
        Vector3 shotDirection = -_camera.GlobalBasis.Z;
        ...
        else if (_currentGun == GunTypes.Quad)
        {
            ...
            foreach (Vector3 bulletVelocity in Globals.CalculateShotgunDirections(shotDirection, Mathf.DegToRad(30), 5, QuadBulletVelocity))
            {
        ...
        if (_remainingAmmo == 0)
        {
            ...
            _gunAnimationPlayer.Connect(
                nameof(OnGunAnimationFinished),
                new Godot.Callable(this, nameof(OnGunAnimationFinished)),
    ...
    {
        Godot.Collections.Array<Godot.Node3D> pickupables = _pickupDetectionArea.GetOverlappingBodies();
    ...
    private void EscapeShip()
    {
        ...
        Godot.Collections.Array<Godot.Node> cannons = GetTree().GetNodesInGroup("Cannons");
        // get closest cannon
        Cannon closestCannon = cannons.OfType<Cannon>().OrderBy(
            c => c.GlobalPosition.DistanceTo(GlobalPosition)).FirstOrDefault();

        if (Mathf.Abs(closestCannon.GlobalPosition.DistanceTo(GlobalPosition)) < 12.0f)
        {
    ...
    {
        private Node3D _killer;
        private Vector3 _lastKillerPosition;

        public DeathInfo(Node3D killer)
        {
            (_killer, _lastKillerPosition) = (killer, killer.GlobalPosition);
        }
